...
shortEnsemblID <- function(long_name){
  eName <- strsplit(long_name, ".", fixed = TRUE) %>% unlist()
  short_name <- eName[1]
  return(short_name)
}

geneExpr <- read_csv(argvs[1]) %>% dplyr::mutate(ensembl_gene_id = map_chr(gene_id, shortEnsemblID)) %>% dplyr::select(- gene_id)
